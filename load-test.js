    ...
    const req = https.get(TARGET_URL, (res) => {
      ...
      res.on('end', () => {
        ...
        if (metrics.totalRequests % 10 === 0) {
          console.log(`Completed ${metrics.totalRequests} requests. Latest latency: ${latency.toFixed(2)}ms`);
        }
...
function calculateStatistics() {
  ...
  metrics.p95Latency = (p95Index >= 0 && p95Index < sortedLatencies.length) ? 
    sortedLatencies[p95Index] : 0;
  
  metrics.p99Latency = (p99Index >= 0 && p99Index < sortedLatencies.length) ? 
...
async function runLoadTest() {
  ...
  console.log(`Running with ${CONCURRENT_USERS} concurrent users for ${TEST_DURATION_SECONDS} seconds`);
