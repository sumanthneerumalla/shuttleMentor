...
export const videoCollectionRouter = createTRPCRouter({
	...
	create: protectedProcedure
		...
		.mutation(async ({ ctx, input }) => {
			...
			if (user.userType === UserType.STUDENT) {
				if (
					input.ownerStudentUserId &&
					input.ownerStudentUserId !== user.userId
				) {
					throw new TRPCError({
						code: "FORBIDDEN",
						message:
				...
				if (input.ownerStudentUserId) {
					...
					if (user.userType === UserType.FACILITY && !isSameClub(user, owner)) {
						throw new TRPCError({
							code: "FORBIDDEN",
							message:
								"Facility users can only create collections for students in their club.",
						});
					}
		...
		.query(async ({ ctx, input }) => {
			...
			if (
				...
				throw new TRPCError({
					code: "FORBIDDEN",
					message:
						"Only admins and facility users can search eligible video collection owners.",
				});
			}
	...
	update: adminProcedure
		...
		.mutation(async ({ ctx, input }) => {
			...
			if (!collection || collection.isDeleted) {
				throw new TRPCError({
					code: "NOT_FOUND",
					message: collection?.isDeleted
						? "Video collection is deleted"
	...
	addMedia: protectedProcedure
		...
		.mutation(async ({ ctx, input }) => {
			...
			const activeMediaCount = collection.media.filter(
				(m: any) => !m.isDeleted,
			).length;
	...
	getAllMediaForCoaches: protectedProcedure.query(async ({ ctx }) => {
		// Get the current user
		const user = await getCurrentUser(ctx);
		...
		.mutation(async ({ ctx, input }) => {
			...
			if (!isOwner && !isAdmin(user) && !isFacilitySameClub) {
				throw new TRPCError({
					code: "FORBIDDEN",
					message:
						"You are not authorized to assign coaches to this collection",
				});
			}
