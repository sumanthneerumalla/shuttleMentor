 ...
 */
export function getCurrentWeekRange(timezone?: string): { startOfWeek: Date; endOfWeek: Date } {
  let now: Date;
 ...
 * @throws Error if the date is invalid
 */
export function getWeekRangeForDate(date: Date, timezone?: string): { startOfWeek: Date; endOfWeek: Date } {
  validateDate(date);
...
export function getWeekNumber(date: Date): number {
  ...
  const daysDiff = Math.floor((target.getTime() - weekStart.getTime()) / 86400000);
 ...
 */
export function areDatesInSameWeek(date1: Date, date2: Date, timezone?: string): boolean {
  validateDate(date1);
  validateDate(date2);
