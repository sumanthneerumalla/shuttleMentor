...
**Status**: Reviewed. Uses Radix Dialog with `max-w-4xl` and `max-h-[60vh]` — acceptable on mobile. `mx-4` added to `DialogContent` globally via `dialog.tsx`. Full-screen migration deferred.
...
Touch target sizes not yet changed. `lg` variant already meets 44px. `default` (40px) and `sm` (36px) are slightly short. Deferred — not blocking.
...
Global CSS rule added to `globals.css` under `@media (max-width: 767px)` — sets `font-size: max(1rem, 16px)` on `input`, `select`, `textarea`. Prevents iOS Safari zoom on focus.
...
> **Status**: Utility classes above not yet added. iOS zoom prevention ✅ done. Touch target min-height rules added globally.
...
**Fix**: `p-6` → `p-4 md:p-6` for tighter mobile padding. **Status: Pending** — not yet applied.
...
| File | Changes |
|---|---|
| `src/app/_components/client/authed/CoachingNoteModal.tsx` | Reviewed — acceptable on mobile with `mx-4` on DialogContent; full-screen migration deferred |
| `src/app/_components/shared/Button.tsx` | Pending — touch target increase deferred |
| `src/styles/globals.css` | ✅ iOS zoom prevention added; touch target min-height added |
| `src/app/_components/shared/dialog.tsx` | ✅ `mx-4` added to DialogContent for mobile margins |
| `src/app/_components/shared/ResourceVideoCard.tsx` | Pending — `p-6` → `p-4 md:p-6` not yet applied |
| `src/app/_components/client/authed/CoachSelector.tsx` | Pending — max-height reduction deferred |
| `src/app/layout.tsx` | Not yet verified |
